  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto pb-12">
      ...
      {showMultiLiveMonitor && createPortal(
        <div className="fixed inset-0 z-[999999] bg-slate-950/90 backdrop-blur-3xl flex flex-col p-4 md:p-8 overflow-hidden animate-in fade-in duration-300">
          <div className="max-w-7xl mx-auto w-full flex flex-col h-full">
            {/* Top Header Bar */}
            <div className="flex items-center justify-between border-b border-white/10 pb-4 mb-6 shrink-0">
              <div className="flex items-center gap-3.5">
                <div className="p-3 rounded-2xl bg-gradient-to-tr from-red-600 via-rose-600 to-pink-600 text-white shadow-lg shadow-red-500/25 border border-red-400/20">
            ...
            {/* Grid Wall */}
            <div className="flex-1 overflow-y-auto custom-scrollbar pr-1 flex flex-col justify-start">
              {(() => {
                ...
                if (activeEmps.length === 0) {
                  return (
                    <div className="my-auto flex flex-col items-center justify-center text-center p-12 text-slate-400 gap-4 bg-slate-900/40 border border-slate-800/60 rounded-3xl backdrop-blur-xl">
                ...
                // Calculate layout columns based on active count for neat alignment
                const gridClass = activeEmps.length === 1
                  ? "grid grid-cols-1 max-w-2xl mx-auto w-full gap-6 items-center"
                  : activeEmps.length === 2
                  ...
                  <div className={gridClass}>
                    {activeEmps.map((emp: any) => {
                      ...
                      return (
                        <div
                          key={emp.user.id}
                          className="bg-slate-900/80 border border-slate-800/80 rounded-2xl p-4 flex flex-col gap-3.5 shadow-2xl hover:border-slate-700/80 backdrop-blur-xl transition-all duration-300"
                        >
